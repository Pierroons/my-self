...
for nom, brut in sources.items():
    d = parse(brut)
    if d is None:
        retards.append(f"{nom} : date illisible (« {brut or 'rien'} »)")
        lignes.append(f"  ✗ {nom:20} illisible : {brut!r}")
    elif d < attendu:
        jours = (today - d).days
        retards.append(f"{nom} : arrêtée au {d.isoformat()}, soit {jours} jours")
        lignes.append(f"  ✗ {nom:20} {d.isoformat()}  RETARD ({jours} j)")
    else:
        lignes.append(f"  ✓ {nom:20} {d.isoformat()}")
...
etat_courant = {n: {"date": str(sources.get(n) or ""), "volume": volumes.get(n)} for n in sources}
