...
```
derived_key = HMAC-SHA256(key = recovery_word, message = material + "|v2" + user_salt)
```

The shipped implementation is [`client/sr-derive.js`](client/sr-derive.js) — load it rather than rewriting it. The frozen test vectors that any reimplementation must reproduce live in [`tests/vecteurs-derivation.json`](tests/vecteurs-derivation.json).

### Derivation material — pick a mode, there is no default

`material` is what binds a fingerprint to a service, and the library **refuses to derive without an explicit mode**. A default would be a choice imposed on every integrator without saying so, and the choice depends on how the service is served and on what is acceptable to lose.

| Mode | `material` is… | What it buys | What it costs |
|------|---------------|--------------|---------------|
| `'hostname'` | `location.hostname`, **read in the browser**, lowercased | Real anti-phishing: a clone of the page served elsewhere derives from *its own* hostname, so the fingerprint it produces is worthless against the real server — and it needs no adaptation to fail | The fingerprint is bound to the hostname. Losing or changing the address loses L2 recovery for every existing account |
| `'label'` | a stable label supplied by the integrator, taken **verbatim** | Accounts survive a change of address | **No anti-phishing at all.** A copy served elsewhere with the same label derives exactly the fingerprint the real server stored |

The material must be **read** in the browser, never received from the network. Material that a server supplies is material that *any* server can supply, including one imitating the real service.
...
|------|-----------|------------|
| Client-side key derivation | HMAC-SHA256 | key = recovery_word, message = material + "&#124;v2" + user_salt |
| Derivation material | `'hostname'` or `'label'` | mandatory — the library has no default and throws if the mode is missing |
...
| Account salt | 16 random bytes, rendered as 32 lowercase hex characters | one per account, generated by the browser at registration (`srEngendrerSel`), stored in clear (a salt is not a secret) — mandatory, the library rejects any other shape |
...
3. **Per-entry HMAC** (an instance key, `SELFRECOVER_SU_AUDIT_SECRET` — separate from the SU passphrase: changing the passphrase does not break the chain; the key itself is rotated with `rotate-audit-key`, which re-signs the log without changing its hashes)
...
- Passive phishing — **in `'hostname'` mode only**; `'label'` mode offers none
...
A companion module applies the same idea to the disk: a **LUKS2** encrypted volume unlocks with a recovery passphrase, without email or third party. It is a separate secret — a diceware passphrase drawn for each machine, derived with Argon2id under the label `disk` — not the memorized word of your web accounts, so neither opens the other.
