...
From LAN (`192.0.2.0/24`, auto-bypass) or with `sj_bypass` cookie :
...
```bash
# After edits, redeploy to DEVSERVER
scp -i $SSH_KEY <file> user@devserver:/tmp/
ssh user@devserver 'sudo cp /tmp/<file> /var/www/bi-self/<path> && sudo chown www-data:www-data /var/www/bi-self/<path>'
```
