...
scp /tmp/backend.tar user@devserver:/tmp/
ssh user@devserver '
...
- **LAN** : IP dans `192.0.2.0/24`, `10.x`, `127.x`, `::1`, `fe80::…` → bypass automatique.
...
ssh user@devserver "echo '$NEW_TOKEN' | sudo tee /var/lib/selfjustice/admin/bypass_token.txt"
...
- **Pas de cloud** : tout vit sur le DEVSERVER. Le jour où le DEVSERVER tombe, la démo
