...
The paper loop is measured end to end: the envelope rendered, rasterised, re-read, reconstituted **byte for byte**, and the vault reopened with the code printed on its page 2. `outils/lire_pli.py` names every missing QR code, writes no partial file, and refuses to conclude when it has no reference fingerprint to compare against. **There is no resolution floor**, contrary to what this README claimed for a long time: measured on 2026-09-09, one envelope failed at 300 and 200 dots per inch and succeeded at 150 and 120. A QR code can hide from one rasterisation and show up in the next, with nothing damaged. The reader therefore insists instead of blaming the scan: a finer sweep first, then other resolutions when the source is a PDF. The bench bounds what can be bounded — 300 dots per inch must pass, 80 must refuse — and additionally measures a **blurred, skewed and speckled** sheet, because an envelope is read by a scanner and no rasterisation looks like one.
