...
Every QR code is printed **once**, and the envelope is only worth anything complete. Double printing was removed on 2026-09-09: both copies carried the **same image**, so they failed together — it protected against a torn page, never against an unreadable code. The pages it frees pay for enlarging the QR codes to **7.83 cm**, which does protect against what actually happens: **11 pages** against 13 for the old 4.2 cm envelope — enlarging costs no pages, it gives two back. That size is not chosen but **computed** — `outils/faire_pli.py` derives it from the module count so that each module lands on exactly 5 pixels at 300 dots per inch. The factor retained is 5 pixels per module because it is the largest width tried, hence the one with the most margin against a scanner's blur: re-measured on 2026-09-10, all four return the 24 codes across four draws and fit in 11 pages. Redundancy that would really protect would need parity fragments — `n` fragments of which `k` suffice — and a format break; that is not done. The reader refuses when two readings of the same rank diverge — two sweeps of the same source, or two draws mixed in the same directory — rather than letting the last one read win silently.

The paper loop is measured end to end: the envelope rendered, rasterised, re-read, reconstituted **byte for byte**, and the vault reopened with the code printed at the head of its technical section. `outils/lire_pli.py` names every missing QR code, writes no partial file, and refuses to conclude when it has no reference fingerprint to compare against. **There is no resolution floor**, contrary to what this README claimed for a long time: measured on 2026-09-09, one envelope failed at 300 and 200 dots per inch and succeeded at 150 and 120. A QR code can hide from one rasterisation and show up in the next, with nothing damaged. The reader therefore insists instead of blaming the scan: a finer sweep first, then other resolutions when the source is a PDF. The bench bounds what can be bounded — 300 dots per inch must pass, 80 must refuse — and additionally measures a **blurred and skewed** sheet, because an envelope is read by a scanner and no rasterisation looks like one. Two blur levels, not one: 0.8 px goes red once the envelope is already unreadable, 1.2 px goes red when it has merely lost its margin — the breaking point is 2.0 px — re-measured on 2026-09-10 on the envelope as it now prints, identical across six draws. What the bench does not establish is printed alongside the rest: no sensor grain, no copier binarisation, no stain, no phone photograph.
