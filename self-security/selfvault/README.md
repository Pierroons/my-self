...
Every QR code is printed **once**, and the envelope is only worth anything complete. Double printing was removed on 2026-09-09: both copies carried the **same image**, so they failed together — it protected against a torn page, never against an unreadable code. The pages it frees pay for enlarging the QR codes to 7 cm, which does protect against what actually happens: **12 pages** against 13 for the old 4.2 cm envelope. Redundancy that would really protect would need parity fragments — `n` fragments of which `k` suffice — and a format break; that is not done. The reader refuses when two readings of the same rank diverge — two sweeps of the same source, or two draws mixed in the same directory — rather than letting the last one read win silently.
